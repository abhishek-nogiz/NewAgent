...
""".strip()
    ...
    def _normalize_meta_description(self, meta_description: str, focus_keyword: str, topic: TrendTopic) -> str:
        ...
        return self._trim_to_length(normalized, 150)
    ...
    def _expand_short_article(
        ...
        min_words = max(300, min(self.config.min_article_words, 320))
        if self._word_count(article_html) >= min_words:
            return article_html

        article_body, sources_tail = self._split_before_sources_section(article_html)
        focus_display = self._display_focus_keyword(focus_keyword or (topic.keyword if topic is not None else "the story"))
        additions: list[str] = []
        ...
        for fact, publisher in self._supporting_fact_candidates(research):
            ...
            if self._word_count(article_body + "".join(additions) + sources_tail) >= min_words:
                break
